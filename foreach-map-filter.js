...
Write a function called onlyEvenValues which 
    ...
    with only the even values in the array 
...
function showFirstAndLast(arr){
    return arr.map(function(word){
        ...
        const lLetter = word.charAt(word.length - 1);
...
Write a function called addKeyAndValue which 
    ...
    function with the new key and value added 
    for each object 
...
function addKeyAndValue(arr,key,value){
    const res = arr.map(function(obj){
        obj[key] = value;
        return obj;
    });
    return res;
...
Write a function called vowelCount 
    ...
    the vowel appears in the string. This 
    function should be case insensitive so a 
...
function vowelCount(str){
    ...
    for(let letter of str){
        ...
        const pos = 'aeiou'.indexOf(letter)
        if(pos != -1){
            if(count[letter]){
                count[letter]++;
            }else {count[letter] = 1;}
...
Write a function called extractKey which 
    accepts an array of objects and some 
    key and returns a new array with the 
...
returns a new array with the value of the 
key with a name of "first" and the value of a 
key with the name of  "last" in each object, 
...
which accepts an array of objects and a 
key and returns a new array with all the 
objects that contain that key.
...
accepts an array and a value and returns 
the first element in the array that has 
the same value as the second parameter or 
...
a key, and some value to search for 
...
all of the vowels (both uppercased and lowercased) 
removed. Every character in the new string should 
be lowercased.
...
Write a function called doubleOddNumbers which 
    ...
    (HINT - you can use map and filter to 
